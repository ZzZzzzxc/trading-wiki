  ...
  return {
    ...
    execute: async (ctx): Promise<DebateResult> => {
      ...
      for (let r = 0; r < rounds; r++) {
        // Bull 发言
        const bullPrompt = `你是拥有15年行业经验的乐观派投研分析师。你的风格是积极但不盲目，每个观点必须引用报告中的具体数据或事实作为支撑。
...
{ "title": "标题", "points": ["观点1（数据支撑）","观点2（数据支撑）","观点3（数据支撑）"], "conclusion": "总结" }`;
        ...
        const bearPrompt = `你是拥有15年行业经验的悲观派投研分析师。你擅长发现乐观逻辑中的漏洞和忽略的风险。
